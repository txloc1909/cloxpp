...
class Test:
            ...
            for i, line in enumerate(f.readlines(), start=1):
                ...
                if match := ERROR_LINE_PATTERN.search(line):
                    ...
                    if not language or language == _suite.language:
                        self._expected_errors.add(
                            f"[line {match[3]}] {match[4]}")
                        self._expected_exit_code = 65
                        _expectations += 1
                    continue
    ...
    def _validate_runtime_error(self, error_lines):
        if len(error_lines) < 2:
            self._fail(
                f"Expected runtime error {self._expected_runtime_error} and got none.")

        if error_lines[0] != self._expected_runtime_error:
            self._fail(
                f"Expected runtime error {self._expected_runtime_error} and got:")
...
def run_suite(name: str) -> bool:
    ...
    for file_ in Path("./e2e_tests").rglob("*.lox"):
        run_test(file_)

    term.clear_line()
    if _n_failed == 0:
        print(
            f"All {term.green(_n_passed)} tests passed ({_expectations} expectations).")
    else:
        print(
            f"{term.green(_n_passed)} tests passed. {term.red(_n_failed)} tests failed.")
...
def _define_test_suites():
    ...
    early_chapters = {
        "e2e_tests/scanning": "skip",
        "e2e_tests/expressions": "skip",
    }
